...
namespace StrongGrid.Resources
{
	...
	public class Contacts : IContacts
	{
		...
		/// </returns>
		public async Task<(string FileName, Stream Stream)[]> DownloadExportFilesAsync(string jobId, bool autoDecompress = false, CancellationToken cancellationToken = default)
		{
			...
			using (var client = new HttpClient(handler))
			{
				for (int i = 0; i < job.FileUrls.Length; i++)
				{
					...
					if (autoDecompress && fileName.EndsWith(gzipExtension))
					{
